...
if rcv_gtp_start == True:
    search4_stat = "ON"
elif rcv_gtp_stop == True:
    search4_stat = "OFF"
else:
    logline="you need to set rcv_gtp_* config parms in the header of this file - with both false no matching is possible"
    logger.error(logline)
                            ...
                            "bool": {
                                "filter": [
                                    ...
                                    },
                                    {
                                        "range": {
                                            "fw_log_time": {
                                                "gte": row["threat_fw_log_time"], "format": elastictimeformat
                                            }
                                        }
                                    }
                                ],
...
#d) write the correlated threats to an own index
        try:
            ...
            index_gtp_x_threat=index_gtp_x_threat+dt.datetime.today().strftime("%Y.%m.%d")
